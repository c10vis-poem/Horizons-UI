...
|---|---|---|---|---|
| 2.1 | GenieX is the decided primary runtime (`geniex serve`, `:18181/v1`). | Already forked to `c10vis-poem/GenieX` (July 14) — do NOT re-fork. `RuntimeDef.builtIns()` has a `geniex` def; **no geniex binary wired into the app** yet; `ort_engine` is the only real daemon the app launches. | **MISSING (wiring, not forking)** | Wire `geniex serve` behind the def — this is really P1.3 applied to a second binary. Device already has `geniex-bench` + GGUF + HTP libs (verify, don't re-download — see device-inventory). |
